...
const StyledTooltip = styled.div.attrs((p) => ({
  bg: p.bg || "dark",
  delay: p.delay || 0.01,
}))`
  position: fixed;
  ...
  .tooltip-arrow {
    ...
    &.tooltip-top {
      ...
      border-top: calc(var(--arrow-size) * 2) solid #555;
    ...
    &.tooltip-bottom {
      border-left: var(--arrow-size) solid transparent;
      border-right: var(--arrow-size) solid transparent;
      border-bottom: calc(var(--arrow-size) * 2) solid #555;
      left: calc(50% - var(--arrow-size));
    ...
    &.tooltip-right {
      ...
      border-right: calc(var(--arrow-size) * 2) solid #555;
    ...
    &.tooltip-left {
      border-top: var(--arrow-size) solid transparent;
      border-left: calc(var(--arrow-size) * 2) solid #555;
      border-bottom: var(--arrow-size) solid transparent;
...
function Tooltip({
  ...
  const posRef = useRef({ x: 0, y: 0, pos: "bottom" });
    ...
    <>
      ...
      {disabled || (
        <Portal>
          ...
          >
            <div
              className={`tooltip-arrow tooltip-${posRef.current.pos}`}
            ></div>
